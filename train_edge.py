...
def train(args,query):
    ...
    checkpt_path = "./trained_models/{}_{}_{}{}_{}/".format(
        config['env_name'],
        config['alg'],
        query['name'],
...
if __name__ == "__main__":
    ...
    argParser.add_argument('--train_config', type=str, default="retrain_analytical_mushr_edge")
    argParser.add_argument('--vertices_file', type=str, default="vertices")
    argParser.add_argument('--edges_file', type=str, default="edges")

    args = argParser.parse_args()

    # parse vertices into dictionary
    exp_vertices_fpath = os.path.join(os.path.dirname(__file__), 'roadmap_files', f'{args.vertices_file}.txt')

    vertices = {}
    for row in np.loadtxt(exp_vertices_fpath ,delimiter=",", dtype=float):
            vertices[int(row[0])] = row[1::]

    edges = []
    # parse edges into list of vertex-tuples
    exp_edges_fpath = os.path.join(os.path.dirname(__file__), 'roadmap_files', f'{args.edges_file}.txt')

    for row in np.loadtxt(exp_edges_fpath ,delimiter=",", dtype=int):
        edges.append({"name":f'{row[0]} to {row[1]}',"start": vertices[row[0]], "goal":vertices[row[1]]})
